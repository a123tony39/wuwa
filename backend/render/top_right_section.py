...
def render_top_right_section(
    ...
    for stat_name in sorted_allowed_stats:
        ...
        img = Image.open(path)
        region = ctx.canvas.crop((stat_total_value_x, stat_total_value_y, stat_total_value_x + img.width, stat_total_value_y + img.height))
        composite = Image.alpha_composite(region, img)
        paste_icon(ctx.canvas, composite, (stat_total_value_x, stat_total_value_y))
        # paste value
